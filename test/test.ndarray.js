...
tape( 'the function has an arity of 4', function test( t ) {
	t.strictEqual( dmaxabssorted.length, 4, 'has expected arity' );
	t.end();
});
...
tape( 'if provided an `N` parameter less than or equal to `0`, the function returns `NaN`', function test( t ) {
	...
	v = dmaxabssorted( 0, x, 1, 0 );
	t.strictEqual( isnan( v ), true, 'returns expected value' );

	v = dmaxabssorted( -1, x, 1, 0 );
	t.strictEqual( isnan( v ), true, 'returns expected value' );
...
tape( 'if provided a `stride` parameter equal to `0`, the function returns the first indexed element', function test( t ) {
	...
	v = dmaxabssorted( x.length, x, 0, 0 );
	t.strictEqual( v, 1.0, 'returns expected value' );
